...
TEMP_DIR = os.path.join(THIS_DIR, "templates/sections_template_default")
DEFAULT_TEMPLATE = u'cv.tex'

MACROS_CONF = {
    "STYLE_DIR": os.path.join(THIS_DIR, "includes/cls"),
    "MACROS": os.path.join(THIS_DIR, "includes/macros"),
    "BIBSTYLE": os.path.join(THIS_DIR, "includes/bibstyle")
}

def get_style(style_path=MACROS_CONF["STYLE_DIR"], style_filename="myRes"):
    return os.path.join(style_path, style_filename)


def get_amcros(macro_path=MACROS_CONF["MACROS"]):
    return [os.path.join(macro_path, f) for f in os.listdir(macro_path)]
...
if __name__ == "__main__":
    ...
    gen_tex_file(get_tex_doc(config=content), filename="test.tex")
